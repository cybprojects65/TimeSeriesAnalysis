...
public class PeriodicityDetector {
	...
	public void demo() throws Exception {
		...
		System.out.println("Signal samples: " + signal.length);
		...
		System.out.println("Detected F:" + F + " indecision [" + lowermeanF + " , " + uppermeanF + "]");
	}
	...
	public double detectFrequency(double[] signal, int samplingRate, float minPossibleFreq, float maxPossibleFreq, float wantedFreqError, int FFTnsamples, boolean display) throws Exception {
		...
		if (wantedFreqError > -1) {
			...
			System.out.println("Suggested pow for window length=" + pow);
		...
		System.out.println("Suggested windows length (samples)=" + wLength);
		System.out.println("Suggested windows length (s)=" + ((float) wLength / (float) samplingRate) + " s");
		...
		System.out.println("Error in the Measure will be: " + error + " Hz");
		System.out.println("A priori Min Freq: " + minPossibleFreq + " s");
		System.out.println("A priori Max Freq: " + maxPossibleFreq + " s");
		...
		System.out.println("Number of frequency peaks " + maxfrequencies.length);
		...
		System.out.println("TimeSeriesAnalysis->Frequency " + meanF);
		System.out.println("TimeSeriesAnalysis->Periodicity " + possibleperiod);
		...
		if ((meanF <= minPossibleFreq) || (meanF >= maxPossibleFreq) || (possibleperiod == 0) || (possibleperiod > (maxperiod))) {
			System.out.println("TimeSeriesAnalysis->Invalid periodicity " + (meanF <= minPossibleFreq) + " , " + (meanF >= maxPossibleFreq) + " , " + (possibleperiod == 0) + " , " + (possibleperiod > (maxperiod)));
			...
			System.out.println("TimeSeriesAnalysis->periodicity is valid " + possibleperiod);
			this.meanF = meanF;
	...
	public LinkedHashMap<String, String> detectAllFrequencies(double[] signal, int samplingRate, float minPossibleFreq, float maxPossibleFreq, float wantedFreqError, int FFTnsamples, float sensitivity, boolean display) throws Exception {
		...
		System.out.println("Frequency Resolution: "+frequencyRes);
		// trace spectrum
		...
		for (int i = 0; i < maxfrequencies.length; i++) {
			...
			for (Double peakFreq : maxfrequencies[i]) {
				...
				{
					System.out.println("DISCREPANCY WITH RESPECT TO THE PREVIOUS FREQ:"+(peakFreq-previousFreq));
					System.out.println("RATIO WITH RESPECT TO THE PREVIOUS FREQ:"+((peakFreq-previousFreq)/error));	
					if (counter == 0) {
					...
					System.out.println("SpectorgramIdx_"+(counter+1)+":" + spectrogramidx);
					System.out.println("Strength_of_Periodicity_"+(counter+1)+":" + signalMaximumAnalyzer.currentSpikesPowerSpectra[i].get(freqCounter));
					System.out.println("Strength_of_Periodicity_Interpretation"+(counter+1)+":" + getPowerSpectrumStregthInterpretation(signalMaximumAnalyzer.currentSpikesPowerSpectra[i].get(freqCounter)));
					System.out.println("Frequency_"+(counter+1)+":" + peakFreq);
					System.out.println("UpperFrequencyConfidence_"+(counter+1)+":" + uppermeanF);
					System.out.println("LowerFrequencyConfidence_"+(counter+1)+":" + lowermeanF);
					System.out.println("Period"+":" + period);
					System.out.println("UpperFrequencyPeriod_"+(counter+1)+":" + (1d / lowermeanF));
					System.out.println("LowerFrequencyPeriod_"+(counter+1)+":"+ (1d / uppermeanF));
					System.out.println("");
					counter++;
