		...
		for (double[] slice : spectrogram) {
			...
			for (int k = 1; k < slice.length; k++) {
				...
				if (!overfirstmin && (slice[k]>slice[k-1])){
					 System.out.println("First minimum in spectrum is at idx "+k);
					overfirstmin=true;
				}
				if (overfirstmin) {
					if (ele > (max + (Math.abs(max) * tolerance))) {
//						 System.out.println(">max up:"+ele +">" +(max + (Math.abs(max) * tolerance))+" at idx "+k);
						max = ele;
						bestidx = k;
					}
			...
			System.out.println("max freq in spec: " + maxs[j]+" index "+minFidx + bestidx);
			...
			System.out.println("max power : " + max + " min power: " + min + " mean " + mean + " power " + averagepower[j]);
			j++;
		}
	...
	public ArrayList<Double>[] takePeaksInSpectrogramFrames(double[][] spectrogram, int samplingRate, int windowSamples, float minfreq) {
		...
		for (int j=0;j<spectrogram.length;j++) {
			...
			double maxAmp = MathFunctions.getMax(slice);
			double minAmp = MathFunctions.getMin(slice);
