...
import it.cnr.timeseries.analysis.datastructures.BigSamplesTable;
import it.cnr.timeseries.analysis.datastructures.ImageTools;
import it.cnr.timeseries.analysis.datastructures.NumericSeriesGraph;
import it.cnr.timeseries.analysis.datastructures.RapidMinerConfiguration;
import it.cnr.timeseries.analysis.datastructures.SpectrumPlot2;
import it.cnr.timeseries.analysis.datastructures.TimeSeriesGraph;
...
public class SignalProcessing {
	...
	public static Image renderSignalWithGenericTime(double[] signal, float t0, float timeshift, String name) {
		if (signal.length > 20000) {
			System.out.println("Too many points to display: " + signal.length);
			return null;
		}
		org.jfree.data.xy.XYSeries xyseries = new org.jfree.data.xy.XYSeries(name);
		float time = t0;
	...
	public static Image renderSignalWithTime(double[] signal, Date[] dates, String name, String format) {
		...
		for (int i = 0; i < signal.length; i++) {
			try {
				...
				System.out.println("Skipping value yet present: " + dates[i]);
			}
		}
	...
	public static Image renderSignalSpectrogram2(double[][] spectrogram) {
		...
		System.out.println("Spectrum W:" + spectrumPlot.width);
		System.out.println("Spectrum H:" + spectrumPlot.height);
	...
	public static double[] downSample(double[] signal, int numElements, RapidMinerConfiguration config) throws Exception {
	...
	public static Date[] fillTimeLine(double[] timemilliseconds, double samplingRate, RapidMinerConfiguration config) throws Exception {
	...
	public static double[] fillTimeSeries(double[] values, double[] timeseconds, double samplingRate, RapidMinerConfiguration config) throws Exception {

		double t0 = timeseconds[0];
		double t1 = timeseconds[timeseconds.length - 1];
		int signalength = Math.abs((int) ((t1 - t0) * samplingRate) + 1);
		System.out.println("SignalProcessing->Old Time Series had: " + values.length + " samples. New Time Series will have: " + signalength + " samples");
	...
	public static void main(String[] args) throws Exception {
		RapidMinerConfiguration conf = new RapidMinerConfiguration();
