package it.cnr.timeseries.analysis.ssa;
...
public class SingularSpectrumAnalysis {
    ...
    private static double[][] transpositionMatrix(double matrix[][]) {
    	System.out.println("SSA->Building a matrix with dimensions: "+matrix[0].length+" X "+matrix.length);
...
//    	List eigenvectors = data.getEigenVectors().subList(0, 11);
    	...
    	System.out.println("SSA: value for L: "+L);
    	int nEigenVectors = eigenvectors.size();
    	System.out.println("Number of Selected Eigenvectors For Reconstruction: "+nEigenVectors);
...
//    			System.out.println("Forecast: "+y.get(j));
    	...
    	System.out.println("Length of the original signal: "+signalSize+" Length of the reconstructed signal: "+y.size());
